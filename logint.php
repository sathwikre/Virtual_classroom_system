...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM usert WHERE username = '$username'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        if (password_verify($password, $row['password'])) {
            echo "<p style='color: green;'>Login successful! Redirecting to home page...</p>";
            header("Refresh: 2; url=mainindexteacher.html"); // Redirect to home page after 1 second
        } else {
